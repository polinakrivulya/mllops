...
**Бизнес-цель**: Автоматическая маршрутизация обращений пользователей в поддержку: приоритетная обработка негативных эмоций (anger, sadness, fear) для снижения среднего времени решения.

**Продакшн-метрики**:
- Качество:
    - Macro F1 на валидации ≥ 0.85 (целевой минимум для учебного проекта)
    - Accuracy ≥ 0.85 (вторичная)
- Технические:
    - P95 latency ≤ 150 мс на CPU (при batch_size=1, последовательная обработка)
    - Доля ошибок инференса ≤ 1%
    - RAM ≤ 250 МБ на инстанс сервиса
- Надежность обучения:
    - Фиксированный random_seed
    - Воспроизводимость результатов ±0.5% F1 (на одном и том же seed)

**Данные:**
- Hugging Face Datasets: dair-ai/emotion
- Преимущества: легальная доступность, небольшой размер, готовая разметка, быстрые загрузка/предобработка
